...
export async function loadModeKriminal(ctx) {
    ...
    try {
        ...
        // 4. Load marker titik koordinat
        await _loadKriminalMarkers(ctx, _buildDomain(tahun, baseDomain));
    } catch (error) {
    ...
    try {
        ...
        const geoLayer = L.geoJSON({ type: "FeatureCollection", features }, {
            ...
            onEachFeature: (feature, layer) => {
                ...
                layer.on('click', (e) => _showKriminalKecamatanPopup(ctx, e, props, layer, tahun, kabProps, kabLayer));
            }
        });

        ctx.kecamatanLayerGroup.addLayer(geoLayer);
        // Load marker titik koordinat dalam kabupaten ini
        await _loadKriminalMarkers(ctx, _buildDomain(tahun, [['kabupaten_id', '=', kabProps.id]]));
...
// ── Popup Kecamatan ──────────────────────────────────────────────────────────
function _showKriminalKecamatanPopup(ctx, e, props, layer, tahun, kabProps, kabLayer) {
    const tahunLabel = tahun ? `Tahun ${tahun}` : 'Semua Tahun';
    const kasusLabel = props.jumlah_kasus > 0
        ? `<strong style="color:${props.color};">${props.jumlah_kasus.toLocaleString('id-ID')} Kasus</strong>`
        : `<strong style="color:#27ae60;">Tidak Ada Kasus</strong>`;
        ...
        setTimeout(() => {
            ...
            if (btn) {
                btn.addEventListener('click', () => {
                    ...
                    drillDownKriminalDesa(ctx, props, layer, tahun, kabProps, kabLayer);
                });
            }
        }, 0);
    });
...
// ════════════════════════════════════════════════════════════════
// LEVEL 3 — DESA (dalam kecamatan terpilih)
// ════════════════════════════════════════════════════════════════
export async function drillDownKriminalDesa(ctx, kecProps, kecLayer, tahun, kabProps, kabLayer) {
    ...
    try {
        // 1. Jumlah kasus per desa dalam kecamatan ini
        const domain = _buildDomain(tahun, [['kecamatan_id', '=', kecProps.id]]);
        const groups = await ctx.orm.call(
            'petadigi.kriminalitas',
            'read_group',
        ...
        // 2. Geometry desa dalam kecamatan ini
        const records = await ctx.orm.searchRead(
            'petadigi.desa',
        ...
        if (features.length === 0) {
            ...
            drillDownKriminalKecamatan(ctx, kabProps, kabLayer, tahun);
            return;
        }

        // 3. Render choropleth desa
        const geoLayer = L.geoJSON({ type: "FeatureCollection", features }, {
            style: (feature) => ({
                color: '#555555', weight: 1.5, opacity: 1,
                fillColor: feature.properties.color, fillOpacity: 0.75,
            ...
            onEachFeature: (feature, layer) => {
                ...
                layer.on('click', (e) => _showKriminalDesaPopup(ctx, e, props, tahun));
            }
        });

        ctx.desaLayerGroup.addLayer(geoLayer);
         // Load marker titik koordinat dalam kecamatan ini
        await _loadKriminalMarkers(ctx, _buildDomain(tahun, [['kecamatan_id', '=', kecProps.id]]));

        // Tombol kembali ke level kecamatan
        _addKriminalBackButton(ctx, 'kecamatan', { kecProps, kecLayer, tahun, kabProps, kabLayer });
...
// ── Popup Desa ────────────────────────────��────────────────────────────────
function _showKriminalDesaPopup(ctx, e, props, tahun) {
        ...
        onAdd: () => {
            ...
            L.DomEvent.on(btn, 'click', async (ev) => {
                ...
                if (targetLevel === 'kabupaten') {
                    ...
                    await drillDownKriminalKecamatan(ctx, backCtx.kabProps, backCtx.kabLayer, backCtx.tahun);
                }
            });
            return btn;
        },
